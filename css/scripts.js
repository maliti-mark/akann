...
function akanNames(){
    ...
    var year=parseInt(dateEntered[0]);
     ...
     if(!isNaN(dayOfWeek)){
         if(gender=="Male"){
             document.getElementById("display").innerHTML= "Your akan names is"+ maleNames[Math.round(dayOfTheWeek)]+" and you were born on "+ daysOfWeek[dayOfWeek];
         }
         else if(gender=="Female"){
             document.getElementById("display").innerHTML="You were born on "+daysOfWeek[dayOfWeek]+ " and Your akan name is " + femaleNames[Math.round(dayOfTheWeek)];
         }else{
